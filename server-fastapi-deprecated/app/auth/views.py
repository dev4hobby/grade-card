...
from app.auth.schemas import Token, LoginForm
from app.auth.config import social_auth, redirect_uri

from app.social_account.schemas import SocialAccountCreate, SocialAccountEmail
from app.social_account.services import (
    update_social_account,
    check_exist_social_auth_account,
    create_social_auth_account,
)

from app.token.services import authenticate_user, create_access_token
from app.user.services import user_verification
...
async def get_access_token(item: LoginForm):
    ...
    data = create_access_token(data={"user_id": user.pk})
    return data
...
@auth_router.get("/github/callback", status_code=201, response_model=None)
async def github_login(request: Request):
    ...
    account = await check_exist_social_auth_account(
        username=profile["login"], account_id=profile["id"]
    )
    if account:
        return create_access_token(data={"user_id": account.user_id})
    if not account:
...
async def github_verification_account(
    request: Request, pk: int, item: SocialAccountEmail
):
    """Verification email in github account"""

    response = await update_social_account(
        request_dict=dict(request), pk=pk, item_dict=item.dict()
    )
    if response:
