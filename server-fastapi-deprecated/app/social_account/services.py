from app.social_account.models import SocialAccount
from app.social_account.schemas import SocialAccountCreate

from app.user.services import get_user_from_email, user_create
from app.user.schemas import UserCreate


async def update_social_account(
    request_dict: dict, pk: int, item_dict: dict
) -> bool or dict:
    ...
    item_dict.update({"user_id": user["id"]})
...
async def check_exist_social_auth_account(
    username: str, account_id: int
) -> dict or bool:
    """Check if social auth account exist in db"""
    account = await SocialAccount.filter(username=username, account_id=account_id).first()
    if not account:
        return False
    return account
