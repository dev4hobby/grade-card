...
from app.auth.services import get_web_url, create_uuid, send_email
from app.user.schemas import UserCreate, UserInDb
...
async def user_create(
    request_dict: dict, item: UserCreate, additional_text: Optional[str] = ""
) -> dict:
    """Creating user"""
...
async def user_update(pk: int, item: UserInDb) -> dict:
    ...
    if item_dict["password"] is not None:
        item_dict.update(
            {"hashed_password": get_hashed_password(password=item_dict["password"])}
        )
...
async def _validate_password(item: dict) -> dict:
    ...
    if password == repeat_password:
        ...
        return item
    raise HTTPException(
        status_code=404, detail="The password and the repeat password didnt match"
    )
